...
export const createLeader = async (req, res) => {
  const { name, position, level, manifesto, county, constituency, ward } =
    req.body;
  ...
  try {
    ...
    if (existingLeader) {
      return res
        .status(409)
        .json({
          message: "Leader already exists for this region and position.",
        });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to create leader", error: err.message });
  }
};
...
export const getLeaders = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch leaders", error: err.message });
  }
};
...
export const getLeaderById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving leader", error: err.message });
  }
};
...
export const updateLeader = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to update leader", error: err.message });
  }
};
...
export const getGlobalLeaderPerformance = async (req, res) => {
  try {
    ...
    const leaderPerformance = leaders.map((leader) => {
      const leaderReviews = reviews.filter(
        (review) => review.leaderId.toString() === leader._id.toString()
      );
      const allScores = leaderReviews.flatMap((review) =>
        Object.values(review.ratings || {}).filter(
    ...
    res
      .status(500)
      .json({
        message: "Failed to calculate leader performance",
        error: err.message,
      });
  }
};
...
export const deleteLeader = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to delete leader", error: err.message });
  }
};
